...
(define (quad-width q)
  (cond
    ...
    [(eq? 'line (quad-name q)) (foldl fl+ 0.0
                                 (for/list : (Listof Float) ([q : USQ (in-list (quad-list q))] #:when (quad? q))
                                   (quad-width q)))]
    [else 0.0]))
...
(define (render-pieces ps)
  (define-values (initial-ps last-p) ((inst split-last Quad) ps))
  ;;bg snoc
  ...
  (λ(qs [measure #f])
    (if (not (empty? qs))
        (let* ([measure (or measure (assert (quad-attr-ref/parameter (car qs) world:measure-key) flonum?))]
          ...
          (map (λ([broken-piece : (Listof Quad)]) (compose-line-proc broken-piece measure-quad-proc)) broken-pieces))
        (list (line '())))))
...
(define (adaptive-fit-proc pieces measure [use-first? #t] [use-best? #t])
  ...
  (cond
    ...
    [else
     ...
     (define result (let loop : (Listof Natural) ([pos : Breakpoint last-position][acc : (Listof Breakpoint) null])
...
(define wrap-best (make-wrap-proc
                           ...
                           quad-width
                           pieces->line
                           (λ([x : (Vectorof Quad)] [y : Float]) (adaptive-fit-proc x y #f #t)))) ; note difference in boolean args
...
(define (add-horiz-positions starting-quad)
  (define-values (new-quads final-width)
    (for/fold ([new-quads : (Listof Quad) empty]
               [width-so-far : Float 0.0])
