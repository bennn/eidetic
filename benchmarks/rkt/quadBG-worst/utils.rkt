...
(define (merge-attrs . quads-or-attrs-or-lists)
  (for/fold : QuadAttrs
           ([acc : QuadAttrs '()])
           ([qs  : QuadAttrs (in-list quads-or-attrs-or-lists)])
    (for/fold : QuadAttrs
              ([acc : QuadAttrs acc])
              ([q : (Pairof Symbol Any) (in-list qs)])
...
(define (flatten-attrs q0 q1)
  ...
  (define-values (x-attrs y-attrs other-attrs-reversed)
    (for/fold ([xas : QuadAttrs null]
               [yas : QuadAttrs null]
               [oas : QuadAttrs null])
              ([attr : Any (in-list all-attrs)])
    ...
    (cond
      [(quad? x)
       (if (null? (quad-list x))
           ...
           ((inst map QEXP Any)
            (λ([xi : Any]) (do-explode xi x)) (quad-list x)))] ; replace quad with its elements, exploded
      [(string? x) ;; it's a string
       (map (λ([xc : String]) (quad world:split-quad-key (make-quadattrs (quad-attrs parent)) (list xc))) (regexp-match* #px"." x))]
...
(define (join-quads qs-in)
  (let ([make-matcher (λ ([base-q : Quad])
    ...
    (let loop ([qs : (Listof Quad) qs-in][acc : (Listof Quad) '()])
      (if (null? qs)
          (reverse acc)
          (let* ([base-q (first qs)]
...
(define (attr-delete qas . ks)
  (filter (λ([qa : (Pairof Symbol Any)]) (not (ormap (λ(k) (equal? (car qa) k)) ks))) qas))
