...
(define (make-pieces qs)
  ...
  (: make-first-pieces ((Listof Quad) -> PieceQuad))
...
(: font-attributes-with-defaults (Quad -> (List Font-Size Font-Name Font-Weight Font-Style)))
...
(: render-piece ((PieceQuad) (Boolean) . ->* . PieceQuad))
...
;; shorthand
(: render-piece-before-break (PieceQuad -> PieceQuad))
...
(: render-word-break (Word-BreakQuad Symbol -> Quad))
...
(: render-pieces ((Listof PieceQuad) -> (Listof PieceQuad)))
...
(: adaptive-fit-proc (((Vectorof PieceQuad) Float) (Boolean Boolean) . ->* . (Listof Breakpoint)))
