...
;; --- bg: replace Trie struct with vector
;(define-struct: (K V) Trie ([opt : (Option V)]
;                            [map : (HashTable K (Trie K V))]))
(define-type (Trie K V)
  (Vector (Option V) (HashTable K (Trie K V))))
(: make-Trie (All (K V) (-> (Option V) (HashTable K (Trie K V)) (Trie K V))))
(define (make-Trie opt map)
  (vector opt map))
(: Trie-opt (All (K V) (-> (Trie K V) (Option V))))
(define (Trie-opt t)
  (vector-ref t 0))
(: Trie-map (All (K V) (-> (Trie K V) (HashTable K (Trie K V)))))
(define (Trie-map t)
  (vector-ref t 1))
;; --- end edit
